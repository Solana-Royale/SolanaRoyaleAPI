...
<div id="authedGame">
  <span>Choose horse:</span>
  <div>
    <input id="numpick" type="number" min="1" max="8"></input>
  </div><br>
  <button onclick="flipHorse()">Flip horse</button><br><br>
  <code id="flipState"></code>
</div>
...
<script>
  function login() {
    ...
    .then(r => {
      ...
      document.getElementById("authBox").style.display = "none";
      console.log("Authorized user", un);
      console.log("Generating horse race game");
      fetch("/api/games/create?gid=horserace&token=" + encodeURIComponent(r.token))
      .then(r => r.json())
      .then(game => {
        if (game.error === true) {
          console.error("Game session failed to generate with error", game.message);
  ...
  function flipHorse() {
    console.log("test")
    var side = document.getElementById('numpick').value;

    var data = {
      selected: parseInt(side)
    }
